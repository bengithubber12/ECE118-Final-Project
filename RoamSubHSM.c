...
static uint8_t curMotorBias = 0; // Motor bias represents left(0) and right(1) sway for the bot
    ...
    switch (CurrentState) {
        case InitPSubState: // If current state is initial Psedudo State
            if (ThisEvent.EventType == ES_INIT)// only respond to ES_Init
            {
                ...
                //Set the initial motor BIAS to left(0)
        ...
        case FREE_ROAM: // in the first state, replace this with correct names
            ...
            run(); // go forward
            //roboSway(curMotorBias); //go forward with bias
        ...
        case TAPE_HANDLER: // Handles tape detection
            ...
            switch (ThisEvent.EventType) {
                case ES_ENTRY:
                    ES_Timer_InitTimer(TURN_TIMER, ONE_SECOND + QUARTER_SECOND);
                    break;
                    
                case ES_EXIT:
                    ES_Timer_SetTimer(TURN_TIMER, ONE_SECOND + QUARTER_SECOND);
                    break;
