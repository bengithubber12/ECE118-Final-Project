...
uint8_t InitTapeService(uint8_t Priority)
{
   ES_Timer_InitTimer(TAPE_SERVICE_TIMER, TIMER_1_TICKS);
    ...
    AD_Init();
    AD_AddPins(AD_PORTW6|AD_PORTW5|AD_PORTW4|AD_PORTW3|AD_PORTW7|AD_PORTW8);
...
ES_Event RunTapeService(ES_Event ThisEvent) {
    ...
    uint16_t TapeBackRight = AD_ReadADPin(AD_PORTW7);
    
    uint16_t TapeBackLeft = AD_ReadADPin(AD_PORTW8);
